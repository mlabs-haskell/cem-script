...
mkConstructorMatch = \case
  ...
  where
    ...
    go name count = do
      ...
      let leftPat = ConP name [] $ VarP <$> namesLeft
      let rightPat = ConP name [] $ VarP <$> namesRight
